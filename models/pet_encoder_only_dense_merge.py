...
class PET(nn.Module):
    ...
    def test_forward(self, samples, features, pos, **kwargs):
        ...
        sparse_mask = (outputs['split_map_sparse'] > 0.5).cpu()
        ...
        if out_sparse is not None:
            ...
            mask_dense = index_sparse & dense_mask
            pred_logits_dense = out_sparse['pred_logits'][mask_dense]
            count_dense0 = get_pred_count(pred_logits_dense)
            count_dense_list.append(count_dense0)

        if out_dense is not None:
            count_dense1 = get_pred_count(out_dense['pred_logits'][index_dense])
            count_dense_list.append(count_dense1)

        div_out['predict_cnt'] = sum(count_dense_list) / len(count_dense_list) + sum(count_sparse_list)
