...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        head_sizes = torch.cat([v["head_sizes"] for v in targets], dim=0).unsqueeze(0)
        ...
        cost_point = cost_point - torch.sqrt((0.25 * head_sizes) * (0.25 * head_sizes))
        cost_point = torch.clamp(cost_point, min=0.0)
