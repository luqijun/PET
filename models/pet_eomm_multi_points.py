...
class PET(nn.Module):
    ...
    def __init__(self, backbone, num_classes, args=None):
        ...
        self.sample_points = self.args.sample_points

        self.class_embed_4x = nn.Linear(hidden_dim, (num_classes + 1) * (self.sample_points+ 1))
        self.coord_embed_4x = MLP(hidden_dim, hidden_dim, 2 * (self.sample_points + 1), 3)

        self.class_embed_8x = nn.Linear(hidden_dim, (num_classes + 1) * (self.sample_points + 1))
        self.coord_embed_8x = MLP(hidden_dim, hidden_dim, 2 * (self.sample_points + 1), 3)
    ...
    def test_forward(self, samples, features, pos, **kwargs):
        ...
        if out_sparse is not None:
            ...
            sparse_mask = sparse_mask.unsqueeze(-1).repeat(1, 1, (self.sample_points + 1)).flatten(1)
            ...
            middle_mask = middle_mask.unsqueeze(-1).repeat(1, 1, (self.sample_points + 1)).flatten(1)
        ...
        if out_dense is not None:
            ...
            dense_mask = dense_mask.unsqueeze(-1).repeat(1, 1, (self.sample_points + 1)).flatten(1)
        ...
        div_out['predict_cnt'] = div_out['predict_cnt'] / (self.args.sample_points + 1)
        div_out['gt_cnt'] = len(kwargs['targets'][0]['points']) / (self.args.sample_points + 1)
    ...
    def pet_forward(self, samples, features, pos, **kwargs):
        ...
        if 'train' in kwargs or (split_map_sparse > 0.5).sum() > 0:
            ...
            expanded_anchor_points = expand_anchor_points(anchor_points, 8)
            ...
            outputs_sparse = self.predict(self.class_embed_8x, self.coord_embed_8x, samples, expanded_anchor_points, hs, 8, **kwargs)
        ...
        if 'train' in kwargs or (split_map_dense > 0.5).sum() > 0:
            ...
            expanded_anchor_points = expand_anchor_points(anchor_points, 4)
            ...
            #     hs = hs[mask].unsqueeze(0)
            #     anchor_points = anchor_points.unsqueeze(0)[mask]
            outputs_dense = self.predict(self.class_embed_4x, self.coord_embed_4x, samples, expanded_anchor_points, hs, 4, **kwargs)
    ...
    def predict(self, class_embed, coord_embed, samples, points_queries, hs, pq_stride, **kwargs):
        ...
        Crowd prediction
        """
        bs, num_feats, _ = hs.shape
        outputs_class = class_embed(hs)
        outputs_class = outputs_class.view(bs, num_feats * (self.sample_points + 1), 2)
        # normalize to 0~1
        outputs_offsets = (coord_embed(hs).sigmoid() - 0.5) * 2.0
        outputs_offsets = outputs_offsets.view(bs, num_feats * (self.sample_points + 1), 2)
        ...
        outputs_points = outputs_offsets + points_queries.unsqueeze(0)
