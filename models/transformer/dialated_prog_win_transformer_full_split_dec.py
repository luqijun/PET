        ...
        for idx, enc_win_size in enumerate(self.enc_win_size_list):
            ...
            for i in range(stride):
                for j in range(stride):
                    ...
                    memeory_win_one, pos_embed_win_one, mask_win_one = enc_win_partition(mem_one, pos_one, mask_one, enc_win_h, enc_win_w)
            ...
            output_split_list = [enc_win_partition_reverse(output_one, enc_win_h, enc_win_w, h // stride, w // stride) for output_one in output_split_list]
...
class WinDecoderTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, num_decoder_blocks=1, num_decoder_layers=2,
        ...
        self.decoder_list = nn.Sequential(*[TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                            return_intermediate=return_intermediate_dec) for _ in range(num_decoder_blocks)])
            ...
            if is_test  and div is not None:
                if idx < len(dec_win_size_list) - 1:
                    ...
                    for i in range(stride):
                        for j in range(stride):
                            ...
                            tgt_win = enc_win_partition_reverse(tgt_win, self.dec_win_h, self.dec_win_w, qH // stride, qW // stride)
                ...
                if idx < len(dec_win_size_list) - 1:
                    ...
                    hs_win_split_list_reverse = [
                        enc_win_partition_reverse(output_one, self.dec_win_h, self.dec_win_w, qH // stride, qW // stride) for output_one
