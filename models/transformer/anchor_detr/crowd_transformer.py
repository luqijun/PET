...
from ..utils import inverse_sigmoid, window_partition


from .row_column_decoupled_attention import MultiheadRCDA

class Crowd_Transformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 num_encoder_layers=6, num_decoder_layers=6, dim_feedforward=1024, dropout=0.,
                 activation="relu", num_feature_levels=3,num_query_position = 300,num_query_pattern=3, num_points=4,
        ...
        self.class_embed = nn.Linear(d_model, num_classes * num_points)
        self.bbox_embed = MLP(d_model, d_model, num_classes * num_points, 3)
    ...
    def _reset_parameters(self):
        ...
        self.class_embed.bias.data = torch.ones(num_classes * self.num_points) * bias_value
    ...
    def forward(self, srcs, masks):
        ...
        #(num_pattern, 256) -> (1, num_pattern, 1, 256) -> (bs, num_pattern, num_position, 256) -> (bs, num_pattern * num_position, 256)
        ...
        for lid, layer in enumerate(self.decoder_layers):
            ...
            if reference.shape[-1] == 4:
                ...
                repeat_num = tmp.shape[-1] // 2
                repeat_reference = reference.repeat(1, 1, repeat_num)
                tmp += repeat_reference
                tmp = tmp.reshape(tmp.shape[0], -1, 2)  # (B, num_points, 2)
                # tmp[..., :2] += reference
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, srcs, src_padding_masks=None, adapt_pos2d=None,
        ...
        if self.attention_type == "RCDA":
            ...
            tgt2 = self.cross_attn((tgt + query_pos_x).permute(1,0, 2), (tgt + query_pos_y).permute(1,0, 2), k_row, k_col,
...
def build_crowd_transformer():

    return Crowd_Transformer(
        d_model=256,
        nhead=8,
